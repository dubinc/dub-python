...
class ListEventsRequestTypedDict(TypedDict):
    ...
    r"""Filter for root domains. Supports advanced filtering: single value, multiple values (comma-separated), or exclusion (prefix with `-`). Examples: `true` (root domains only), `false` (regular links only), `true,false` (both). If undefined, return both."""
    sale_type: NotRequired[str]
    r"""Filter sales by type. Valid values: `new` (first-time purchases), `recurring` (repeat purchases). Supports advanced filtering: single value, multiple values (comma-separated), or exclusion (prefix with `-`). Examples: `new`, `new,recurring`, `-recurring`. If undefined, returns both."""
